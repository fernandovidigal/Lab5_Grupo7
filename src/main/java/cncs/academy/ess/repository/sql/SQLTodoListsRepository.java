...
public class SQLTodoListsRepository implements TodoListsRepository {
    ...
    public int save(TodoList todoList){
        try (Connection connection = dataSource.getConnection()){
            PreparedStatement stmt = connection.prepareStatement("INSERT INTO lists (name, owner_id) VALUES (?, ?)", Statement.RETURN_GENERATED_KEYS);
    ...
    public void update(TodoList todoList){
        try (Connection connection = dataSource.getConnection()){
            PreparedStatement stmt = connection.prepareStatement("UPDATE lists SET name = ?, owner_id = ? WHERE id = ?");
            stmt.setString(1, todoList.getName());
            stmt.setInt(2, todoList.getOwnerId());
